...
public class SimpleTimeMemLogger {
    ...
    public void after(final AbstractConsumerStage<?> filter, final String idFilterExecution) {
        ...
        // calculate diffs
        final long diffTime = time - Long.parseLong(table.getColumn(row, 1).getValue());
        final long diffMemory = usedMemory - Long.parseLong(table.getColumn(row, 3).getValue());
    ...
    public void close() {
        ...
        for (final String filter : this.tablesFilter.keySet()) {
            ...
            try {
                final String fileName = this.logOutput + "_" + filter + "_" + this.dateFormatter.format(new Date())
                        + ".csv";
