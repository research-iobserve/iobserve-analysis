 ...
 * currently as fall-back behavior, since there is now way to check whether or not the connection is
...
public final class TNetworkLink extends AbstractConsumerStage<TraceMetadata> {
    ...
    protected void execute(final TraceMetadata event) {
        ...
        TNetworkLink.collectUnLinkedResourceContainer(resEnv).stream().forEach(unLinkedResCont -> {
        ...
        // .forEach(link ->
        // link.getConnectedResourceContainers_LinkingResource().add(unLinkedResCont));
        // }
        // }
