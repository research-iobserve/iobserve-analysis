 ...
 *
 */
public class InfrastructureInterfaceProvider extends AbstractInterfaceProvider {

    public InfrastructureInterfaceProvider(final GraphDatabaseService graph) {
        super(graph);
        this.setInterfaceLabel(Label.label("InfrastructureInterface"));
    }

    // TODO wont work with circles so far
    @Override
    public Interface readComponent(final String entityName) {
        try (Transaction tx = this.getGraph().beginTx()) {
            final InfrastructureInterface inter = RepositoryFactory.eINSTANCE.createInfrastructureInterface();
            ...
            // Node eNode;
            // for (final Relationship r : iNode.getRelationships()) {
            // eNode = r.getEndNode();
            //
