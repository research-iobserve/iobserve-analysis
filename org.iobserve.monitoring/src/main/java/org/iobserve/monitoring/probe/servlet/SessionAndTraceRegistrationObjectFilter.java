...
public class SessionAndTraceRegistrationObjectFilter implements Filter, IMonitoringProbe {
	...
	/** Request mapper. */
	private static final Map<String, Integer> pathMap =  new ConcurrentHashMap<String, Integer>();
	...
	public void doFilter(final ServletRequest request, final ServletResponse response, final FilterChain chain) throws IOException, ServletException {
		if (CTRLINST.isMonitoringEnabled()) {
			...
			if (newTrace) {
				...
				CTRLINST.newMonitoringRecord(new ServletTraceHelper(trace.getTraceId(), 
						request.getRemoteHost(), request.getRemotePort(), path));
			}
			...
			Integer objectId = pathMap.get(path);
			if (objectId == null) {
				idCounter++;
				objectId = idCounter;
				pathMap.put(path, objectId);
			}
			
			try {
				CTRLINST.newMonitoringRecord(new BeforeOperationObjectEvent(TIMESOURCE.getTime(), traceId, trace.getNextOrderId(),
						operationSignature, componentSignature, objectId));
				chain.doFilter(request, response);
				CTRLINST.newMonitoringRecord(new AfterOperationObjectEvent(TIMESOURCE.getTime(), traceId, trace.getNextOrderId(),
						operationSignature, componentSignature, objectId));
			} catch (final Throwable th) { // NOPMD NOCS (catch throw is ok here)
				CTRLINST.newMonitoringRecord(new AfterOperationFailedObjectEvent(TIMESOURCE.getTime(), traceId, trace.getNextOrderId(),
						operationSignature, componentSignature, th.toString(), objectId));
