...
abstract class PcmRepresentative<E extends NamedElement> {
	...
	private def void store() {
		/** Vielleicht nicht benötigt, da generierte Dateien im CorrespondenceModel für iObserve möglicherweise uninteressant */
		if (entity instanceof Entity) {
			for (e : generatedFiles) {
				if (e instanceof IJCompilationUnit) {
					val implementationArtifactId = String.format("%s.%s", e.packageName, e.compilationUnitName)
					CorrespondenceModelGenerator.INSTANCE.createCorrespondence(entity,implementationArtifactId)
				}
			}
		}
