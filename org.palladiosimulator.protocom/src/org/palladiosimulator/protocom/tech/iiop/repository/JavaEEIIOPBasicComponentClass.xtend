...
class JavaEEIIOPBasicComponentClass extends JavaEEIIOPClass<BasicComponent> {
	...
	override methods() {
		...
		results += pcmEntity.serviceEffectSpecifications__BasicComponent.map [
			val m = new JMethod().withName(JavaNames::serviceName(it.describedService__SEFF)).withReturnType(
	...
	override jeeClassDependencyInjection() {
		...
		//BUG-FIX PROVISORIAL: https://sdqbuild.ipd.kit.edu/jira/browse/PROTOCOM-4
		val mapBasicComponentAssemblyConnectors = new HashMap
		for(asmConnector : assemblyConnectors) {
			if (asmConnector.requiredRole_AssemblyConnector.requiringEntity_RequiredRole.equals(pcmEntity)) {
				mapBasicComponentAssemblyConnectors.put(asmConnector.entityName, asmConnector)
			}
		}
		
		val basicComponentAssemblyConnectors = mapBasicComponentAssemblyConnectors.values
//		BUG: https://sdqbuild.ipd.kit.edu/jira/browse/PROTOCOM-4
//		val basicComponentAssemblyConnectors = assemblyConnectors.
//			filter[it.requiredRole_AssemblyConnector.requiringEntity_RequiredRole.equals(pcmEntity)]
