 ...
 * @since 1.10
 */
public class TestGeneratedSessionStartEvent extends AbstractGeneratedKiekerTest {

	public TestGeneratedSessionStartEvent() {
		// empty default constructor
	}

	/**
	 * Tests {@link SessionStartEvent#TestSessionStartEvent(long, string)}.
	 */
	@Test
	public void testToArray() { // NOPMD (assert missing)
		for (int i=0;i<ARRAY_LENGTH;i++) {
			// initialize
			SessionStartEvent record = new SessionStartEvent(LONG_VALUES.get(i % LONG_VALUES.size()), STRING_VALUES.get(i % STRING_VALUES.size()));
			
			// check values
			Assert.assertEquals("SessionStartEvent.timestamp values are not equal.", (long) LONG_VALUES.get(i % LONG_VALUES.size()), record.getTimestamp());
			Assert.assertEquals("SessionStartEvent.sessionId values are not equal.", STRING_VALUES.get(i % STRING_VALUES.size()) == null?"":STRING_VALUES.get(i % STRING_VALUES.size()), record.getSessionId());
	...
	public void testEquality() {
		int i = 0;
		SessionStartEvent oneRecord = new SessionStartEvent(LONG_VALUES.get(i % LONG_VALUES.size()), STRING_VALUES.get(i % STRING_VALUES.size()));
		i = 0;
		SessionStartEvent copiedRecord = new SessionStartEvent(LONG_VALUES.get(i % LONG_VALUES.size()), STRING_VALUES.get(i % STRING_VALUES.size()));
	...
	public void testUnequality() {
		...
		SessionStartEvent oneRecord = new SessionStartEvent(LONG_VALUES.get(i % LONG_VALUES.size()), STRING_VALUES.get(i % STRING_VALUES.size()));
		i = 2;
		SessionStartEvent anotherRecord = new SessionStartEvent(LONG_VALUES.get(i % LONG_VALUES.size()), STRING_VALUES.get(i % STRING_VALUES.size()));
