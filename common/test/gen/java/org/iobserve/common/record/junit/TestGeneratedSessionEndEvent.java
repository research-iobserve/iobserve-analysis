...
public class TestGeneratedSessionEndEvent extends AbstractGeneratedKiekerTest {
	...
	public TestGeneratedSessionEndEvent() {
		// empty default constructor
	}

	/**
	 * Tests {@link SessionEndEvent#TestSessionEndEvent(long, string)}.
	 */
	@Test
	public void testToArray() { // NOPMD (assert missing)
		for (int i=0;i<ARRAY_LENGTH;i++) {
			// initialize
			SessionEndEvent record = new SessionEndEvent(LONG_VALUES.get(i % LONG_VALUES.size()), STRING_VALUES.get(i % STRING_VALUES.size()));
			
			// check values
			Assert.assertEquals("SessionEndEvent.timestamp values are not equal.", (long) LONG_VALUES.get(i % LONG_VALUES.size()), record.getTimestamp());
			Assert.assertEquals("SessionEndEvent.sessionId values are not equal.", STRING_VALUES.get(i % STRING_VALUES.size()) == null?"":STRING_VALUES.get(i % STRING_VALUES.size()), record.getSessionId());
	...
	public void testEquality() {
		int i = 0;
		SessionEndEvent oneRecord = new SessionEndEvent(LONG_VALUES.get(i % LONG_VALUES.size()), STRING_VALUES.get(i % STRING_VALUES.size()));
		i = 0;
		SessionEndEvent copiedRecord = new SessionEndEvent(LONG_VALUES.get(i % LONG_VALUES.size()), STRING_VALUES.get(i % STRING_VALUES.size()));
	...
	public void testUnequality() {
		...
		SessionEndEvent oneRecord = new SessionEndEvent(LONG_VALUES.get(i % LONG_VALUES.size()), STRING_VALUES.get(i % STRING_VALUES.size()));
		i = 2;
		SessionEndEvent anotherRecord = new SessionEndEvent(LONG_VALUES.get(i % LONG_VALUES.size()), STRING_VALUES.get(i % STRING_VALUES.size()));
