 ...
 * @since 1.0
 */
public class TestGeneratedServletTraceHelper extends AbstractGeneratedKiekerTest {

	public TestGeneratedServletTraceHelper() {
		// empty default constructor
	}

	/**
	 * Tests {@link ServletTraceHelper#TestServletTraceHelper(long, string, int, string)}.
	 */
	@Test
	public void testToArray() { // NOPMD (assert missing)
		for (int i=0;i<ARRAY_LENGTH;i++) {
			// initialize
			ServletTraceHelper record = new ServletTraceHelper(LONG_VALUES.get(i % LONG_VALUES.size()), STRING_VALUES.get(i % STRING_VALUES.size()), INT_VALUES.get(i % INT_VALUES.size()), STRING_VALUES.get(i % STRING_VALUES.size()));
			
			// check values
			Assert.assertEquals("ServletTraceHelper.traceId values are not equal.", (long) LONG_VALUES.get(i % LONG_VALUES.size()), record.getTraceId());
			Assert.assertEquals("ServletTraceHelper.host values are not equal.", STRING_VALUES.get(i % STRING_VALUES.size()) == null?"":STRING_VALUES.get(i % STRING_VALUES.size()), record.getHost());
			Assert.assertEquals("ServletTraceHelper.port values are not equal.", (int) INT_VALUES.get(i % INT_VALUES.size()), record.getPort());
			Assert.assertEquals("ServletTraceHelper.requestURI values are not equal.", STRING_VALUES.get(i % STRING_VALUES.size()) == null?"":STRING_VALUES.get(i % STRING_VALUES.size()), record.getRequestURI());
	...
	public void testEquality() {
		int i = 0;
		ServletTraceHelper oneRecord = new ServletTraceHelper(LONG_VALUES.get(i % LONG_VALUES.size()), STRING_VALUES.get(i % STRING_VALUES.size()), INT_VALUES.get(i % INT_VALUES.size()), STRING_VALUES.get(i % STRING_VALUES.size()));
		i = 0;
		ServletTraceHelper copiedRecord = new ServletTraceHelper(LONG_VALUES.get(i % LONG_VALUES.size()), STRING_VALUES.get(i % STRING_VALUES.size()), INT_VALUES.get(i % INT_VALUES.size()), STRING_VALUES.get(i % STRING_VALUES.size()));
	...
	public void testUnequality() {
		...
		ServletTraceHelper oneRecord = new ServletTraceHelper(LONG_VALUES.get(i % LONG_VALUES.size()), STRING_VALUES.get(i % STRING_VALUES.size()), INT_VALUES.get(i % INT_VALUES.size()), STRING_VALUES.get(i % STRING_VALUES.size()));
		i = 2;
		ServletTraceHelper anotherRecord = new ServletTraceHelper(LONG_VALUES.get(i % LONG_VALUES.size()), STRING_VALUES.get(i % STRING_VALUES.size()), INT_VALUES.get(i % INT_VALUES.size()), STRING_VALUES.get(i % STRING_VALUES.size()));
