...
for(k in 0:9) {
	for(i in 1:length(experimentSizes)) {
		path <- 'D:\\Dokumente\\Uni\\HiWi\\iobserve\\iobserve-analysis\\experiments\\scalability_experiment\\results\\raw results\\equal_events_X_users\\'
		experimentNr <- k
...
pdf("D:\\Dokumente\\Uni\\HiWi\\iobserve\\iobserve-analysis\\experiments\\scalability_experiment\\results\\TPreprocess_median_equal_events.pdf") 

plot(experimentSizes, TPreprocessMedianMs, log="xy", axes=FALSE, type = "o", main = expression('T'[Preprocess]), ylim = c(1, 100000), xlab="Number of Users", ylab="Median of Execution Time [ms]")
...
pdf("D:\\Dokumente\\Uni\\HiWi\\iobserve\\iobserve-analysis\\experiments\\scalability_experiment\\results\\TRuntimeUpdate_median_equal_events.pdf") 

plot(experimentSizes, TRuntimeUpdateMedian, log="xy", axes=FALSE, type = "o", main = expression('T'[RuntimeUpdate]), ylim = c(10,1000000), xlab="Number of Users", ylab="Median of Execution Time [ms]")
